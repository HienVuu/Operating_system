...
int main (void)
{
  ...
  /* Tính toán và in thông báo về CPU sử dụng */
  int64_t total_ticks = timer_ticks();
  extern int64_t idle_ticks;  // Biến được định nghĩa trong threads/thread.c
  int cpu_usage = (int)(((total_ticks - idle_ticks) * 100) / total_ticks);
  printf ("CPU usage at boot: %d%% of system capacity\n", cpu_usage);
